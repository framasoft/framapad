...
$(document).ready(() => {
  ...
  $('#classic-pad-name').on('focusout', function nameCleaning() {
  ...
  $.getJSON('https://framastats.org/autresStats/framapad/statistics.json', (data) => {
    ...
    $('#expiration').on('change', function shJauge() {
      $('.jauge').hide(); $(`.${this.value}.jauge`).show();
    });
  });

  // Create pad
  $('#create-pad').on('submit', () => {
    const padName = ($('#classic-pad-name').val() === '') ? randomPadName() : $('#classic-pad-name').val();
    let instanceId = $('#expiration').val();

    if ($('#beta').is(':checked')) {
      instanceId = 'beta';
    }

    const padURL = `https://${instanceId}.framapad.org/p/${padName}`;

    $('#pad-url').attr('href', padURL);
